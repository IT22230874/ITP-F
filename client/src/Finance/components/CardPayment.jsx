...
function CardPayment() {
  ...
  useEffect(() => {
  const storedRequestDetails = localStorage.getItem('requestDetails');
  if (storedRequestDetails) {
    const parsedDetails = JSON.parse(storedRequestDetails);
    if (parsedDetails.amount && !isNaN(parseFloat(parsedDetails.amount))) {
      setAmount(parseFloat(parsedDetails.amount));
    } else {
      console.log("Amount not found or invalid in requestDetails", parsedDetails);
    }
  } else {
    console.log("requestDetails not stored", storedRequestDetails);
  }
}, []);
  ...
  <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
    <div class="mx-auto max-w-5xl">
      ...
      <div class="mt-6 sm:mt-8 lg:flex lg:items-start lg:gap-12">
        <form onSubmit={handleSubmit} class="w-full rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:p-6 lg:max-w-xl lg:p-8">
          <div class="mb-6 grid grid-cols-2 gap-4">
            <div class="col-span-2 sm:col-span-1">
              ...
              <input type="text" id="full_name" value={source} onChange={(e) => setSource(e.target.value)} class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500" placeholder="Bonnie Green" required />
            </div>
          ...
          <button type="submit" class="flex w-full items-center justify-center rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4  focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Pay now</button>
        </form>
...
<dt className="text-base font-normal text-gray-500 dark:text-gray-400">Net Amount</dt>
<dd className="text-base font-medium text-gray-900 dark:text-white">${amount}</dd>
</dl>


              <dl class="flex items-center justify-between gap-4">
                <dt class="text-base font-normal text-gray-500 dark:text-gray-400">Tax</dt>
                <dd class="text-base font-medium text-green-500">-${(amount * 0.05)}</dd>
              </dl>
...
<dd className="text-base font-bold text-gray-900 dark:text-white">${amount - (amount * 0.05)}</dd>
</dl>
          </div>
