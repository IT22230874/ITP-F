...
const addEmployee = async (req, res, next) => {
    try {
        ...
        return res.json({ message: "Employee added successfully", data: newEmployee });
    } catch (error) {
...
const removeMemberFromGroup = async (req, res, next) => {
    try {
        ...
        return res.json({message: "upgrouping successfull", data: removedGroupEmployee });
    } catch (error) {
...
const getAttendance = async (req, res, next) => {
    try {
        ...
        const currentData = await AttendenceModel.findOne({ empid, date: formattedDate });
        ...
        if (!currentData || !currentData.arrival) {
            ...
            const departureDateTime = new Date(0, 0, 0, departureHour, departureMinute);

            const millisecondsDifference = departureDateTime.getTime() - arrivalDateTime.getTime();
            ...
            res.json({ message: 'Departure time recorded successfully', attendanceType });
        }
    } catch (error) {
...
const attendanceAnalysis = async (req, res, next) => {
    try {
        ...
        const fullDay = await AttendenceModel.countDocuments({ empid: id, attendencetype: "full day" });
        const halfDay = await AttendenceModel.countDocuments({ empid: id, attendencetype: "half day" });
        const leaves = await AttendenceModel.countDocuments({ empid: id, attendencetype: "leave" });
...
const getAttendanceSummary = async () => {
    try {
        ...
        return { fullDayPercentage, halfDayPercentage, leavePercentage, numberOfEmployees };
    } catch (error) {
